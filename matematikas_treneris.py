...
class IelogosanasLapa(Frame): # Ielogošanās lapa, kur lietotājs ievada savus datus
    ...
    def __init__(self, parent, controller): # Inicializē klases īpašības un mainīgos
        ...
        Label(frame, text="Ielogoties", font=('calibri', 20, 'bold')).pack()
        
        Label(frame, text="Lietotājvārds:", font=('calibri', 15, 'bold')).pack(pady=5) # Lietotājvārda ievades etiķete
        
        # Ievades lauks lietotājvārda ievadei
        self.username_entry = Entry(frame, textvariable=controller.username, justify=CENTER, font=('calibri', 10, 'bold'))
        
        self.username_entry.pack(pady=2.5) # Iepako ievades lauku
        
        Label(frame, text="Parole:", font=('calibri', 15, 'bold')).pack(pady=2.5) # Paroles ievades etiķete
        
        self.password_entry = Entry(frame, show="*", justify=CENTER, font=('calibri', 10, 'bold')) # Ievades lauks parolei (slēpts)
        
        self.password_entry.pack(pady=2.5) # Iepako ievades lauku
        
        
        # Poga, kas izsauc pieslēgšanās funkciju
        Button(frame, text="Ielogoties",
                command=lambda: self.pareizi_pieslegties(controller)).pack(pady=2.5)
        
        # Poga, kas parada lapu lietotāja reģistēšanai
        Button(frame, text="Uz Reģistrācijas Lapu",
                command=lambda: (controller.paradit_lapu(RegistracijasLapa),
                                 ...
                                 self.password_entry.delete(0, END))).pack(pady=2.5) # Vizuāli notira paroles ievades lauku
    ...
    def pareizi_pieslegties(self, controller):
        ...
        with sqlite3.connect(datu_baze) as savienojums: #Izveido savienojumu
            ...
            if curs.fetchone(): # Ja tiek atgriezts viens pieprasījuma elements
                ...
                messagebox.showinfo("Veiksmīgi", "Ielogošanās Ir Veiksmīga!")
                ...
                # Parada kļudu
                messagebox.showerror("Kļūda", "Nepareizs lletotājvārds Vai Parole!")
...
class Sakumlapa(Frame): # Sākuma lapa
    ...
    def __init__(self, parent, controller): # Inicializē klases īpašības un mainīgos
        ...
        # Parada virsraksts
        Label(frame, text="Matemātikas Treneris",
                    font=('calibri', 20, 'bold')).pack()
        
        # Poga, lai sāktu darbu
        Button(frame, text="Sākt Darbu", 
                  command=lambda: controller.paradit_lapu(GalvenaLapa)).pack(pady=2.5)
        
        # Izveido pogu atgriešanai ielogošanas lapā
        Button(frame, text="Atpakaļ", command=lambda:
                    controller.paradit_lapu(IelogosanasLapa)).pack(pady=2.5)
        
        # Poga "Rediģēt"
        self.rediget_poga = Button(frame, text="Rediģēt",
                                        command=lambda: controller.paradit_lapu(AdminLapa))
                
        self.rediget_poga.pack(pady=5) # Sākumā tiek pievienota, bet to paslēpsim atkarībā no lietotāja
    ...
    def atjaunot_lapu(self): # Pārbauda, vai lietotājs ir admin, un attiecīgi rādām pogu
        ...
        if self.controller.username.get() == "admin": # ja ir lietotājs ir admins
            ...
            self.rediget_poga.pack(pady=5)  # Rāda pogu "Rediģēt"
...
class GalvenaLapa(Frame): # Galvenā lapa, kur lietotājs izvēlas grūtības līmeni
    ...
    def __init__(self,parent, controller): # Inicializē klases īpašības un mainīgos
        ...
        # Parada virsrakstu
        Label(frame, text="Izvēlēties Grūtības Līmeni",
                    font=('calibri', 20, 'bold')).pack()
        
        varianti = ["viegls", "videjais", "gruts"] # Grūtības līmeņu saraksts
        
        Style().theme_use('clam') # Definē izmantoto tēmu visām lapām 
        
        # Definē krasas aspektus laukam komboboks
        Style().configure("TCombobox", fieldbackground= "green", background= "white")
        
        # Kombinētais lauks grūtības līmeņa izvēlei
        self.komboboks = Combobox(frame,width=27, values=varianti,
                                  textvariable=controller.selected_difficulty)
        
        self.komboboks.pack(pady=5) # Iepako komboboks lauku
        
         # Poga "Turpināt"                
        Button(frame, text="Paradīt Uzdevumu",
                    command=lambda: self.parbaudit_un_paradit(controller)).pack(pady=2.5)
        
        # Izveido pogu atgriešanai sakuma lapā
        Button(frame, text="Atpakaļ", command=lambda:
                    controller.paradit_lapu(Sakumlapa)).pack(pady=5)
        
        # Poga "Pabeigt darbu"
        Button(frame, text="Pabeigt Darbu", command=controller.quit).pack(pady=2.5) 
...
class AtbildesLapa(Frame): # Lapa, kurā tiek attēlota atbilde
    ...
    def parbaudit_atbildi(self, controller):
        ...
        zinojums = messagebox.askyesno("Jautājums",
                                        "Vai Jūsu Atbilde Ir Tāda Pati Kā Uz Ekrāna?")
        
        if zinojums: # Ja atbilde ir "Jā"
            
            #Izvada ziņojumu
            messagebox.showinfo("Paziņojums", "Jūs Esat Malacis!")
            
        else: # Ja atilde ir "Nē"
            
            #Izvada ziņojumu
            messagebox.showinfo("Paziņojums", "Jums Jātrenējas Vairāk!")
...
class RegistracijasLapa(Frame): # Izveido klasi RegistracijasLapa, logu reģistrācijas forma
    ...
    def __init__(self, parent, controller): # Inicializē klases īpašības un mainīgos
        ...
        Label(frame, text="Reģistrācija", font=('calibri', 20, 'bold')).pack()  # Izveido virsraksta uzrakstu

        Label(frame, text="Lietotājvārds:", font=('calibri', 15, 'bold')).pack(pady=5) # Izveido uzrakstu lietotājvārdam
        
        self.username_entry = Entry(frame, justify=CENTER, font=('calibri', 10, 'bold')) # Izveido ievades lauku lietotājvārdam
        
        self.username_entry.pack(pady=2.5) # Iepako ievades lauku lietotājvārdam

        Label(frame, text="Parole:", font=('calibri', 15, 'bold')).pack(pady=2.5) # Izveido uzrakstu paroles ievadei
        
        self.password_entry = Entry(frame, show="*", justify=CENTER, font=('calibri', 10, 'bold'))  # Izveido paroles ievades lauku ar maskētiem simboliem
        
        self.password_entry.pack(pady=2.5) # Attēlo paroles ievades lauku
        
        Style().configure('TButton', font = ('calibri', 15, 'bold'), # Pievieno pogam fontu un teksta izmēru 
                                        borderwidth = '4')
        
        # Pievieno pogas mainīšanu atkarības no pēles pozicijas visās lapās
        ...
        Button(frame, text="Reģistrēties",
               command=lambda: self.registret_lietotaju(controller)).pack(pady=2.5)
    ...
    def registret_lietotaju(self, controller):
        ...
        if username.lower() != "admin": # Ja admina vēl nav, tad to var pievienot
            ...
            with sqlite3.connect(datu_baze) as savienojums: # Izveido savienijumu ar datu bāzi
                ...
                messagebox.showinfo("Veiksmīgi", "Lietotājs ir Veiksmīgi Reģistrēts!")
...
class AdminLapa(Frame): # Izveido klasi adminam
    ...
    def __init__(self, parent, controller): # Inicializē klases īpašības un mainīgos
        ...
        Label(frame, text="Pievienot Jaunu Uzdevumu", font=('calibri', 20, 'bold')).pack() # Izveido virsrakstu jaunam uzdevumam
        
        Label(frame, text="Uzdevums:", font=('calibri', 15, 'bold')).pack(pady=5) # Izveido tekstu, kas apzīmē uzdevuma ievades lauku
        
        self.uzdevums_entry = Entry(frame) # Izveido ievades lauku, kur lietotājs var ierakstīt uzdevumu
        
        self.uzdevums_entry.pack(pady=2.5) # Iepako ievades lauku logā
        
        Label(frame, text="Atbilde:", font=('calibri', 15, 'bold')).pack(pady=5) # Izveido tekstu, kas apzīmē atbildes ievades lauku
        
        self.atbilde_entry = Entry(frame) # Izveido ievades lauku atbildei
        
        self.atbilde_entry.pack(pady=2.5) # Iepako ievades lauku logā
        
        Label(frame, text="Sarežģītība:", font=('calibri', 15, 'bold')).pack(pady=5) # Izveido tekstu, kas apzīmē sarežģītības ievades lauku
        
        self.sarezgitiba_entry = Entry(frame) # Izveido ievades lauku sarežģītības līmeņa ievadīšanai
        
        self.sarezgitiba_entry.pack(pady=2.5) # Iepako ievades lauku logā
        
        # Izveido pogu uzdevuma saglabāšanai un piesaista funkciju
        Button(frame, text="Saglabāt", command=self.pievienot_uzdevumu).pack(pady=5) 
        
        # Izveido pogu atgriešanai galvenajā lapā
        Button(frame, text="Atpakaļ", command=lambda:
                    controller.paradit_lapu(Sakumlapa)).pack(pady=2.5)
    ...
    def pievienot_uzdevumu(self): # Saglabā jaunu uzdevumu datubāzē
        ...
        if uzdevums and atbilde and sarezgitiba: # Pārbauda, vai visi lauki ir aizpildīti
            ...
            messagebox.showinfo("Veiksmīgi", "Uzdevums Ir Ieiksmīgi Pievienots!")
